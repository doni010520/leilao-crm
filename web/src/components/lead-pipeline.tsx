...
const COLUMNS: { estagio: LeadEstagio; title: string; dot: string; head: string }[] = [
  ...
  { estagio: "em_negociacao", title: "Negociação", dot: "bg-blue-500", head: "text-blue-700" },
...
export function LeadPipeline({ leads }: { leads: PipelineOverview[] }) {
  ...
  return (
    <div className="flex h-[calc(100vh-180px)] gap-3 overflow-x-auto pb-4 lg:grid lg:grid-cols-6 lg:overflow-x-hidden">
      {COLUMNS.map((col) => {
        const items = leads.filter((l) => l.estagio === col.estagio);
        return (
          <div key={col.estagio} className="flex min-h-0 w-[260px] shrink-0 flex-col rounded-card bg-stone-50/70 lg:w-auto">
                ...
                return (
                  ...
                  >
                    <div className="flex items-start gap-2">
                      <div className="flex h-8 w-8 shrink-0 items-center justify-center rounded-full bg-stone-200 text-[10px] font-semibold text-stone-600">
                        {initials || "?"}
                      </div>
                    ...
                    <div className="mt-2 flex flex-wrap gap-1">
                      ...
                      <span className="rounded-full bg-stone-100 px-2 py-0.5 text-[9px] text-ink-soft">
                    ...
                    <select
                      ...
                      className="mt-2 w-full rounded border border-stone-200 bg-stone-50 px-1.5 py-1 text-[10px] text-ink-soft"
                    >
                      {COLUMNS.map(c => (
