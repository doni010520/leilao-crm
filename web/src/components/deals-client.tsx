...
const STATUS_MAP: Record<DealStatus, { label: string; cls: string }> = {
  ...
  desistiu: { label: "Desistiu", cls: "bg-gray-100 text-gray-600" },
};
  ...
  return (
    <>
      <PageHeader
        ...
        action={<Button onClick={() => setShowForm(true)}><Plus size={16} /> Novo negócio</Button>}
      />
      <div className="mb-6 grid grid-cols-2 gap-3 md:grid-cols-4">
        <Card className="text-center"><p className="text-2xl font-bold text-ink">{initialDeals.length}</p><p className="text-xs text-ink-soft">Total</p></Card>
        <Card className="text-center"><p className="text-2xl font-bold text-blue-600">{ativos.length}</p><p className="text-xs text-ink-soft">Ativos</p></Card>
        <Card className="text-center"><p className="text-2xl font-bold text-green-600">{initialDeals.filter(d => d.status === "arrematou").length}</p><p className="text-xs text-ink-soft">Arrematados</p></Card>
        <Card className="text-center"><p className="text-2xl font-bold text-ink">{fmt(total)}</p><p className="text-xs text-ink-soft">Valor pipeline</p></Card>
      </div>
      {initialDeals.length === 0 ? (
        <EmptyState title="Nenhum negócio" hint="Crie um negócio vinculando um lead a um imóvel." />
      ) : (
        <div className="space-y-3">
          {initialDeals.map(deal => {
            const s = STATUS_MAP[deal.status] ?? STATUS_MAP.acompanhando;
            return (
              <Card key={deal.id} className="flex items-center gap-4">
